...
mysql -uroot -e "use radius; SELECT username FROM usadas WHERE min <= DATE_SUB(CURDATE(), INTERVAL $Days day) and groupname like '%Pausada'" |sort > /tmp/pausados.txt
...
num=0
cat /tmp/pausados.txt | while read users
do
num=$[$num+1]
